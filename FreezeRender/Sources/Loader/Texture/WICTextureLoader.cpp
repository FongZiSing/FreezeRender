...
namespace Detail
{
	namespace Loader
	{
		std::vector<ExtesionDescriptor> targets = {
...
WICTextureLoader::WICTextureLoader(const std::filesystem::path& filepath)
{
	...
	int index = IsExtesionEqual(this->filepath, Detail::Loader::targets);
	this->extension = index >= 0 ? (Extension)Detail::Loader::targets[index].reserved1 : Extension::Unknown;
	this->status = Verify();
}
...
WICTextureLoader::Status WICTextureLoader::Load(Texture* result, APixelFormat asformat)
{
	...
	Status status = Detail::Loader::Load(this->GetAbsolutePath().Data(), result, asformat);
	if (status != Status::Ok)
	{
	...
	else
	{
		...
		result->id = WideStringView(L"WICLoaderMesh_").Append(GenerateId(id));
	}
