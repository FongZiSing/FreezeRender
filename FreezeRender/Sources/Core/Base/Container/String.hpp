...
#pragma endregion string_functor
	...
	/**
	 * @brief Implemention of string view.
	 */
	template <typename CharType>
	...
	public:
		...
		constexpr BasicStringView& TrimLeft(const std::locale& loc = std::locale()) noexcept
		{
			Super::remove_prefix([this, &loc]()
				{
					auto predicate = StringClassifiedFunctor<CharType>(std::ctype_base::space, loc);
	...
	public:
		...
		{
			auto predicate = StringTransformFunctor<CharType>(loc);
