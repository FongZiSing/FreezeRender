...
void D2DApp::CreateWindowSizeDependentResources()
{
	...
	else
	{
		...
		swapChainDesc.Width = appData.width;
		swapChainDesc.Height = appData.height;
	...
	if (SUCCEEDED(hr))
	{
		m_pD2DDeviceContext->CreateBitmap(
			D2D1::SizeU(appData.width, appData.height),
...
void D2DApp::UpdateScene(const float& deltaTime)
{
	if (m_pD2DDeviceContext && m_pD2DRenderTarget)
	{
		HPAINTRESULT result = HandlePaintEvent(deltaTime);
		m_pD2DRenderTarget->CopyFromMemory(&appData.rect, result.bits, result.width * 4);
...
{
	Tick(deltaTime);
	UpdateScene(g_timer.GetDeltaTime());
	CalculateFrameStats();
}
...
HRESULT D2DApp::OnCreate()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = HandleCreateEvent(appData.width, appData.height);
	}
...
LRESULT D2DApp::OnActivate(const WPARAM& nFlags, const HWND& hwnd)
{
	...
	{
		appData.bLeftMousePressing = false;
		appData.bMiddleMousePressing = false;
		appData.bRightMousePressing = false;
		appData.bResizing = false;
		appData.keys.Clear();
	}
	return FALSE;
}
...
LRESULT D2DApp::OnLeftMouseDown(const WPARAM& nFlags, const int& x, const int& y)
{
	...
	appData.bLeftMousePressing = true;
...
LRESULT D2DApp::OnMiddleMouseDown(const WPARAM& nFlags, const int& x, const int& y)
{
	...
	appData.bMiddleMousePressing = true;
...
LRESULT D2DApp::OnRightMouseDown(const WPARAM& nFlags, const int& x, const int& y)
{
	...
	appData.bRightMousePressing = true;
...
LRESULT D2DApp::OnSize(const WPARAM& nFlags, const UINT& width, const UINT& height)
{
	...
	appData.rect.right = width;
	appData.rect.bottom = height;
	if (nFlags == SIZE_MAXIMIZED)
	{
		appData.bWindowMaximum = true;
		CreateWindowSizeDependentResources();
		HandleResizeEvent(width, height);
	}
	else if (nFlags == SIZE_RESTORED && appData.bWindowMaximum)
	{
		appData.bWindowMaximum = false;
		CreateWindowSizeDependentResources();
		HandleResizeEvent(width, height);
	}
	return FALSE;
...
LRESULT D2DApp::OnExitSizeMove()
{
	...
	if (appData.bResizing)
	{
		appData.bResizing = false;
		CreateWindowSizeDependentResources();
		HandleResizeEvent(appData.width, appData.height);
	}
	return FALSE;
}
...
HRESULT D2DApp::Initialize(HINSTANCE hInstance, int nCmdShow)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		RECT windowRect = { 0, 0, LONG(appData.width), LONG(appData.height) };
...
LRESULT D2DApp::WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (pThis)
	{
		...
		case WM_RBUTTONUP:      return pThis->OnLeftMouseUp(wParam, LOWORD(lParam), HIWORD(lParam));
